...
We can re-use the `PostItem` component for this, but now we want to update the `PostItem` component to hide the action buttons on the single post page.

PostItem.js should now look like this:
...
Also, we need to create a new component `components/post.Post.js` which should look like this:
...
#### 53. Add comment to post

Now we create a new `components/post/CommentForm.js` to be our compoonent for creating a comment on a post. It's very similar to the PostForm component.
...
#### 54. Display comments and delete

The next component we need to create is one to show the comments in the discussion. Create `components/post/CommentItem.js` as follows:
